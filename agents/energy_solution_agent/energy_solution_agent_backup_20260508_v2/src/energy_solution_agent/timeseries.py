...
def to_hourly_profile(values: list[float], annual_target_mwh: float | None = None, fallback_peak_kw: float | None = None) -> list[float]:
    if values:
        ...
        if len(numeric) == 24:
            profile = numeric
        elif len(numeric) % 24 == 0:
            chunk = len(numeric) // 24
            profile = [sum(numeric[i * chunk : (i + 1) * chunk]) / chunk for i in range(24)]
        elif len(numeric) > 24:
            chunk = max(1, len(numeric) // 24)
            profile = []
            for i in range(0, len(numeric) - len(numeric) % chunk, chunk):
                window = numeric[i : i + chunk]
                profile.append(sum(window) / len(window) if window else 0.0)
            # Distribute any remaining data points across existing buckets
            # to avoid silently discarding them.
            remainder = len(numeric) % chunk
            if remainder and profile:
                trailing = numeric[-remainder:]
                if len(trailing) < len(profile):
                    for j, val in enumerate(trailing):
                        profile[j] = (profile[j] * chunk + val) / (chunk + 1)
                else:
                    logger.warning(
                        "to_hourly_profile discarding %d trailing data points (%d > %d buckets)",
...
def scale_hourly_profile(profile: list[float], annual_target_mwh: float | None = None, peak_kw: float | None = None) -> list[float]:
    if not profile:
        return [0.0] * 24
    values = [float(v) for v in profile[:24] if v is not None]
    if len(values) < 24:
...
def expand_daily_profile_to_year(
    ...
    profile = [float(v) for v in daily_profile_kw[:24] if v is not None]
    if len(profile) < 24:
