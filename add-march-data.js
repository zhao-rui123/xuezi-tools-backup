...
// 读取原始文件
let content = fs.readFileSync('priceData.js', 'utf8');

// 为每个省份添加3月数据
for (const [province, prices] of Object.entries(marchPrices)) {
  // 查找该省份的 "2": { ... } 数据块
  const pattern = new RegExp(`("${province}":\\s*\\{[^}]+"2026":\\s*\\{[^}]+"2":\\s*\\{[^}]+\\})`);
  ...
  content = content.replace(pattern, '$1' + marchData);
}

// 保存
fs.writeFileSync('priceData.js', content);
console.log('已添加2026年3月电价数据');
