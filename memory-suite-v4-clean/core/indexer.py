...
class IndexManager:
    ...
    def update_index(self) -> Dict[str, Any]:
        ...
        try:
            # 获取所有记忆文件
            all_memory_files = list(self._memory_dir.glob("*.md"))

            # 优先索引今日文件
            today_str = datetime.now().strftime("%Y-%m-%d")
            today_file = self._memory_dir / f"{today_str}.md"

            # 排序：今日文件优先，然后按修改时间倒序
            def sort_key(f):
                if f == today_file:
                    return (0, 0)  # 今日文件排第一
                return (1, -f.stat().st_mtime)  # 其他文件按时间倒序

            memory_files = sorted(all_memory_files, key=sort_key)[:self._max_files]
