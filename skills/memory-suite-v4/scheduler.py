...
class Scheduler:
    ...
    def _register_default_tasks(self):
        ...
        # 核心任务 - 每日初始化（00:00创建新记忆文件）
        self.register_task(Task(
            name="daily-init",
            description="每日初始化：创建新的记忆文件",
            handler=self._task_daily_init,
            enabled=self._is_task_enabled("daily-init", True)
        ))
    ...
    def _task_daily_init(self, **kwargs) -> Any:
        """每日初始化任务：创建新的记忆文件"""
        logger.info("执行每日初始化任务...")
        try:
            from core.session_capture import SessionCapture
            capture = SessionCapture(MEMORY_DIR)
            memory_file = capture.ensure_daily_file_exists()
            logger.info(f"每日记忆文件已准备：{memory_file}")
            return {"memory_file": str(memory_file)}
        except Exception as e:
            logger.error(f"每日初始化失败：{e}")
            # 降级处理：直接创建文件
            ...
            try:
                with open(memory_file, 'w', encoding='utf-8') as f:
                    ...
                    f.write("- 新的一天开始\n\n")
                    f.write("## 会话记录\n\n")
                logger.info(f"每日记忆文件已创建（降级模式）：{memory_file}")
                return {"memory_file": str(memory_file), "mode": "degraded"}
            except Exception as e2:
                logger.error(f"降级创建也失败：{e2}")
