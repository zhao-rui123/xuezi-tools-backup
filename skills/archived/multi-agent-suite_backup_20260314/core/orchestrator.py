...
class MultiAgentOrchestrator:
        ...
        if config_file.exists():
            ...
            for agent_data in config.get('agents', []):
                ...
                if agent_id not in self.agents:
                    ...
                    print(f"  ✅ 加载Agent: {agent.name} ({agent.role.value if hasattr(agent.role, "value") else agent.role})")
        else:
            # 如果没有配置文件，使用默认agents
            default_agents = [
    ...
    def save_state(self):
        ...
        for k, v in self.agents.items():
            agent_dict = asdict(v)
            agent_dict['role'] = v.role.value if hasattr(v.role, 'value') else v.role  # 转换枚举为字符串
    ...
    def execute_subtask(self, subtask_id: str) -> bool:
        ...
        try:
            # 构建任务指令
            task_prompt = f"""
你是 {agent.name} ({agent.role.value if hasattr(agent.role, "value") else agent.role})，负责开发任务。
...
"""
    ...
    def list_agents(self):
        ...
        for agent in self.agents.values():
            ...
            print(f"{agent.id:<10} {agent.name:<10} {agent.role.value if hasattr(agent.role, "value") else agent.role:<12} {agent.status:<10} {task.title if task else '-'}")
...
def main():
    ...
    if args.create_task:
        ...
        if agent:
            print(f"\n👤 Agent: {agent.name}")
            print(f"   ID: {agent.id}")
            print(f"   角色: {agent.role.value if hasattr(agent.role, "value") else agent.role}")
