...
BLUE2 = PatternFill('solid', fgColor='2F75B6')
...
def sc(cell, val, fill=None, bold=False, size=10, color='000000', halign='center', wrap=True):
    """Safe cell setter - always sets fill to avoid serialization issues"""
    cell.value = val
    cell.fill = fill if fill else WHITE_FILL
    cell.font = Font(bold=bold, size=size, color=color)
    cell.alignment = Alignment(horizontal=halign, vertical='center', wrap_text=wrap)
    cell.border = TB

def hdr(cell, val, fill=None, size=11):
    cell.value = val
    cell.fill = fill if fill else BLUE
...
sc(ws1['A3'], '工商业储能市场开拓分析报告', fill=WHITE_FILL, bold=True, size=28, color='1F4E79')

ws1.merge_cells('A4:E4')
ws1.row_dimensions[4].height = 30
sc(ws1['A4'], '2026年3月 | 政策解读 | 经济账测算 | 省份梯队 | 全国投资地图', size=13, color='666666')

ws1.merge_cells('A6:E6')
ws1.row_dimensions[6].height = 35
hdr(ws1['A6'], '目  录', size=15)
...
for i, (sheet, title, desc) in enumerate(dirs, 7):
    ...
    sc(ws1.cell(i,1), f'{i-6}. {title}', fill=WHITE_FILL, bold=True, size=12, halign='left')
    sc(ws1.cell(i,2), f'→ {desc}', fill=WHITE_FILL, size=10, color='666666', halign='left')
...
sc(ws2['A1'], '模式一：纯储能投资经济账（10MWh储能）', fill=BLUE, bold=True, size=18, color='FFFFFF')

ws2.merge_cells('A3:D3')
ws2.row_dimensions[3].height = 35
sc(ws2['A3'], '⚠️ 核心门槛结论', fill=BLUE2, bold=True, size=14, color='FFFFFF')
...
for i, (label, val, note) in enumerate(rows, 4):
    ...
    sc(ws2.cell(i,1), label, fill=YELLOW, bold=True)
    ws2.merge_cells(f'B{i}:C{i}')
    sc(ws2.cell(i,2), val, fill=GREEN, bold=True, size=14, color='006100')
    ws2.merge_cells(f'D{i}:L{i}')
    sc(ws2.cell(i,4), note, fill=WHITE_FILL, halign='left')

ws2.merge_cells('A7:L7')
ws2.row_dimensions[7].height = 25
sc(ws2['A7'], '一、基础参数', fill=BLUE2, bold=True, size=12, color='FFFFFF')
...
for i, row in enumerate(params, 8):
    ...
    for j, v in enumerate(row):
        c = ws2.cell(i, j+1)
        sc(c, v, fill=LIGHT if j%2==0 else WHITE_FILL, bold=(j%2==0))

# 敏感性分析
ws2.merge_cells('A13:L13')
ws2.row_dimensions[13].height = 25
sc(ws2['A13'], '二、敏感性分析 - 不同电价差下的IRR与回收期', fill=BLUE2, bold=True, size=12, color='FFFFFF')

pd_list = [0.40, 0.50, 0.55, 0.58, 0.60, 0.65, 0.67, 0.70, 0.80, 0.90, 1.00]
hdrs = ['电价差(元/kWh)'] + [str(p) for p in pd_list]
...
def calc_payback(pd):
    ...
    for y in range(1, 11):
        ...
        if cum >= 0: return f'{y}年'
    return '>10年'

ws2.cell(15,1).value = 'IRR'
sc(ws2.cell(15,1), 'IRR', fill=LIGHT, bold=True)
for j, pd in enumerate(pd_list, 2):
    irr = calc_irr(pd)
    c = ws2.cell(15, j)
    sc(c, f'{irr:.2%}', fill=GREEN if irr>=0.10 else (YELLOW if irr>=0.08 else RED), bold=True, color='006100' if irr>=0.10 else ('9C6500' if irr>=0.08 else '9C0006'))

ws2.cell(16,1).value = '静态回收期'
sc(ws2.cell(16,1), '静态回收期', fill=LIGHT, bold=True)
for j, pd in enumerate(pd_list, 2):
    pb = calc_payback(pd)
    yr = int(pb.replace('年','')) if pb[-1]=='年' else 999
    c = ws2.cell(16, j)
    sc(c, pb, fill=GREEN if yr<=7 else (YELLOW if yr<=10 else RED), bold=True, color='006100' if yr<=7 else ('9C6500' if yr<=10 else '9C0006'))
...
sc(ws2['A19'], '三、全国各省模式一IRR预估（电价差→IRR）', fill=BLUE2, bold=True, size=12, color='FFFFFF')

province_m1 = [
    ('山西','0.40~1.50',0.74,'✅第一梯队',GREEN),
    ('广东','0.40~1.29',0.71,'✅第一梯队',GREEN),
    ('浙江','0.35~1.27',0.69,'✅第一梯队',GREEN),
    ('山东','0.35~0.57',0.55,'⚠️第二梯队',YELLOW),
    ('江苏','0.30~0.60',0.52,'⚠️第二梯队',YELLOW),
    ('河南','0.30~0.55',0.50,'⚠️第二梯队',YELLOW),
    ('湖北','0.30~0.55',0.50,'⚠️第二梯队',YELLOW),
    ('安徽','0.30~0.55',0.50,'⚠️第二梯队',YELLOW),
    ('河北','0.30~0.55',0.50,'⚠️第二梯队',YELLOW),
    ('蒙西','0.25~0.46',0.44,'🔶第三梯队',RED),
    ('甘肃','0.25~0.46',0.44,'🔶第三梯队',RED),
    ('辽宁','0.25~0.45',0.42,'🔶第三梯队',RED),
    ('陕西','0.25~0.50',0.45,'🔶第三梯队',RED),
    ('宁夏','0.25~0.45',0.42,'🔶第三梯队',RED),
    ('青海','0.20~0.40',0.38,'🔶第三梯队',RED),
    ('新疆','0.20~0.45',0.40,'🔶第三梯队',RED),
    ('贵州','0.20~0.45',0.40,'🔶第三梯队',RED),
    ('云南','0.20~0.45',0.40,'🔶第三梯队',RED),
    ('西藏','0.10~0.35',0.30,'🔶第三梯队',RED),
]

ph = ['省份','峰谷价差','IRR估算','梯队','模式一IRR≥10%?','回收期≤7年?']
...
for i, (name, pd_range, irr_est, tier, tier_fill) in enumerate(province_m1, 21):
    ...
    vals = [name, pd_range, f'{irr_est:.2%}', tier, irr_ok, pb_ok]
    for j, v in enumerate(vals, 1):
        c = ws2.cell(i, j)
        if j == 4:
            sc(c, v, fill=tier_fill, bold=True)
        elif j in (5,6):
            sc(c, v, fill=GREEN if '✅' in v else (YELLOW if '⚠️' in v else RED), bold=True, color='006100' if '✅' in v else ('9C6500' if '⚠️' in v else '9C0006'))
        else:
            sc(c, v)
...
sc(ws3['A1'], '模式二：光储一体化投资经济账（1MW + 3MWh储能）', fill=BLUE, bold=True, size=18, color='FFFFFF')

ws3.merge_cells('A3:H3')
ws3.row_dimensions[3].height = 30
sc(ws3['A3'], '🏆 最优配置：1MW光伏 + 3MWh储能（光伏充电利用率98%，性价比最优）', fill=DARK_GREEN, bold=True, size=13, color='FFFFFF')
...
for i, (label, val, note) in enumerate(m2, 4):
    ...
    sc(ws3.cell(i,1), label, fill=YELLOW, bold=True)
    ws3.merge_cells(f'B{i}:C{i}')
    sc(ws3.cell(i,2), val, fill=GREEN, bold=True, size=14, color='006100')
    ws3.merge_cells(f'D{i}:H{i}')
    sc(ws3.cell(i,4), note, fill=WHITE_FILL, halign='left')

ws3.merge_cells('A7:H7')
ws3.row_dimensions[7].height = 25
sc(ws3['A7'], '一、基础参数', fill=BLUE2, bold=True, size=12, color='FFFFFF')
...
for i, row in enumerate(p2, 8):
    ...
    for j, v in enumerate(row):
        sc(ws3.cell(i,j+1), v, fill=LIGHT if j%2==0 else WHITE_FILL, bold=(j%2==0))

ws3.merge_cells('A15:H15')
ws3.row_dimensions[15].height = 25
sc(ws3['A15'], '二、敏感性分析 - 不同储能卖电价的IRR（1MW+3MWh）', fill=BLUE2, bold=True, size=12, color='FFFFFF')
...
sc(ws3.cell(17,1), 'IRR', fill=LIGHT, bold=True)
for j, sp in enumerate(sp_list, 2):
    irr = calc_m2_irr(sp)
    sc(ws3.cell(17,j), f'{irr:.2%}', fill=GREEN if irr>=0.10 else (YELLOW if irr>=0.08 else RED), bold=True, color='006100' if irr>=0.10 else ('9C6500' if irr>=0.08 else '9C0006'))

ws3.row_dimensions[17].height = 20

ws3.merge_cells('A20:H20')
ws3.row_dimensions[20].height = 25
sc(ws3['A20'], '三、多配置对比（1MW光伏配不同储能）', fill=BLUE2, bold=True, size=12, color='FFFFFF')
...
for i, row in enumerate(cmp, 21):
    ...
    for j, v in enumerate(row, 1):
        ...
        if i == 21:
            ...
            sc(c, v, fill=GREEN, bold=True)
        elif '❌' in str(v):
            sc(c, v, fill=RED, bold=True)
        else:
            sc(c, v)

ws3.merge_cells('A27:H27')
ws3.row_dimensions[27].height = 25
sc(ws3['A27'], '四、全国各省模式二IRR预估', fill=BLUE2, bold=True, size=12, color='FFFFFF')

m2_provinces = [
    ('河北','⭐第一梯队',GREEN,'光伏装机大，午间低电价明确'),
    ('山东','⭐第一梯队',GREEN,'分布式光伏全国第二，尖峰机制完善'),
    ('河南','⭐第一梯队',GREEN,'户用光伏全国第一，2025底试运行'),
    ('安徽','⭐第一梯队',GREEN,'工商业分布式全国前三'),
    ('江苏','⭐第一梯队',GREEN,'分布式光伏全国第一，2025底试运行'),
    ('山西','⭐第一梯队',GREEN,'新能源大省，午间低谷明确'),
    ('浙江','⚠️第二梯队',YELLOW,'光伏装机大，现货市场成熟'),
    ('广东','⚠️第二梯队',YELLOW,'珠三角价差高，现货已开'),
    ('蒙西','⚠️第二梯队',YELLOW,'新能源大省，午间低电价明显'),
    ('湖北','⚠️第二梯队',YELLOW,'光伏装机中等，现货已开'),
    ('上海','⚠️第二梯队',YELLOW,'峰谷价差全国最高，光伏资源弱'),
    ('福建','⚠️第二梯队',YELLOW,'峰谷价差较大'),
    ('四川','⚠️第二梯队',YELLOW,'水电为主，气象联动尖峰'),
    ('湖南','⚠️第二梯队',YELLOW,'尖峰时段较长'),
    ('陕西','⚠️第二梯队',YELLOW,'新能源快速发展'),
    ('天津','⚠️第二梯队',YELLOW,'京津冀一体化'),
    ('北京','⚠️第二梯队',YELLOW,'基本无光伏资源'),
    ('海南','⚠️第二梯队',YELLOW,'独立地理单元'),
    ('江西','⚠️第二梯队',YELLOW,'深谷机制完善'),
    ('辽宁','🔶第三梯队',RED,'东北电网，冬季复杂'),
    ('吉林','🔶第三梯队',RED,'风电为主，峰谷差低'),
    ('黑龙江','🔶第三梯队',RED,'峰谷差低，风电为主'),
    ('宁夏','🔶第三梯队',RED,'现场消纳要求高'),
    ('甘肃','⚠️第二梯队',YELLOW,'现货价格低，新能源强配储'),
    ('重庆','🔶第三梯队',RED,'山地地形，分布式有限'),
    ('贵州','🔶第三梯队',RED,'峰谷调整中，分布式受限'),
    ('云南','🔶第三梯队',RED,'水电为主，套利空间有限'),
    ('青海','🔶第三梯队',RED,'峰谷差极低，集中式为主'),
    ('新疆','🔶第三梯队',RED,'集中式为主，弃光问题'),
    ('西藏','🔶第三梯队',RED,'电网薄弱，市场未开'),
    ('内蒙古东部','🔶第三梯队',RED,'与蒙西分开管理'),
]
...
for i, (name, tier, tier_fill, note) in enumerate(m2_provinces, 29):
    ...
    sc(ws3.cell(i,1), name)
    sc(ws3.cell(i,2), tier, fill=tier_fill, bold=True)
    sc(ws3.cell(i,3), note, halign='left')
...
sc(ws4['A1'], '各省电力现货市场价格数据（2025-2026年）', fill=BLUE, bold=True, size=16, color='FFFFFF')

hdrs4 = ['省份','午间低电价','晚高峰','峰谷价差','负电价','模式一IRR≥10%?','模式一回收≤7年?']
for j, h in enumerate(hdrs4, 1):
    hdr(ws4.cell(3, j), h, size=10)
...
for i, row in enumerate(mkt, 4):
    ...
    for j, v in enumerate(row, 1):
        c = ws4.cell(i, j)
        if j == 6:
            sc(c, v, fill=GREEN if '✅' in v else (YELLOW if '勉强' in v else RED), bold=True, color='006100' if '✅' in v else ('9C6500' if '勉强' in v else '9C0006'))
        elif j == 7:
            sc(c, v, fill=GREEN if '✅' in v else (YELLOW if '勉强' in v else RED), bold=True, color='006100' if '✅' in v else ('9C6500' if '勉强' in v else '9C0006'))
        else:
            sc(c, v)
...
sc(ws4['A10'], '说明：电价单位为元/kWh；数据来源：各省电力交易中心2025-2026年公开数据', size=9, color='666666', halign='left')
...
sc(ws5['A1'], '全国31省储能市场梯队划分（2026年）', fill=BLUE, bold=True, size=16, color='FFFFFF')

# 模式一
ws5.merge_cells('A3:H3')
ws5.row_dimensions[3].height = 28
sc(ws5['A3'], '模式一梯队（纯储能10MWh）', fill=DARK_GREEN, bold=True, size=13, color='FFFFFF')
...
sc(ws5['A10'], '模式二梯队（光储一体化1MW+3MWh）', fill=DARK_GREEN, bold=True, size=13, color='FFFFFF')
...
sc(ws5['A17'], '全国31省详细梯队对照表', fill=BLUE2, bold=True, size=13, color='FFFFFF')
...
sc(ws6['A1'], '全国投资地图 - 七大区域省份梯队分布', fill=BLUE, bold=True, size=16, color='FFFFFF')
...
sc(ws6['A12'], '图例说明', fill=BLUE2, bold=True, size=12, color='FFFFFF')

legends = [
    ('✅第一梯队（绿色）','山西/广东/浙江/山东/江苏/河南/安徽/河北/湖北，可立即开拓，IRR预计≥10%'),
    ('⚠️第二梯队（黄色）','上海/福建/四川/湖南/陕西等，市场待激活，有一定套利空间，IRR预计5~10%'),
    ('🔶第三梯队（红色）','甘肃/贵州/云南/青海/新疆/西藏/吉林/黑龙江等，暂不推荐，IRR预计<5%'),
    ('空白','该区域无对应梯队省份'),
]
for i, (txt, note) in enumerate(legends, 13):
    ws6.row_dimensions[i].height = 22
    if i == 13:
        sc(ws6.cell(i,1), txt, fill=GREEN, bold=True)
    elif i == 14:
        sc(ws6.cell(i,1), txt, fill=YELLOW, bold=True)
    elif i == 15:
        sc(ws6.cell(i,1), txt, fill=RED, bold=True)
    else:
        sc(ws6.cell(i,1), txt, bold=True)
    ws6.merge_cells(f'B{i}:F{i}')
    sc(ws6.cell(i,2), note, halign='left')

# 重点开拓省份汇总
ws6.merge_cells('A19:F19')
ws6.row_dimensions[19].height = 28
sc(ws6['A19'], '⭐ 重点开拓省份汇总（双第一梯队）', fill=DARK_GREEN, bold=True, size=13, color='FFFFFF')
...
ph3 = ['省份','核心优势','优先级']
for j, h in enumerate(ph3, 1):
    hdr(ws6.cell(20, j), h)

for i, row in enumerate(priority, 21):
    ws6.row_dimensions[i].height = 25
    sc(ws6.cell(i,1), row[0], fill=GREEN, bold=True)
    sc(ws6.cell(i,2), row[1], halign='left')
    sc(ws6.cell(i,3), row[2], fill=GREEN, bold=True)
...
sc(ws7['A1'], '综合结论与开拓建议', fill=BLUE, bold=True, size=16, color='FFFFFF')
...
conclusions = [
    ...
    ('风险提示', '1.电价差收窄风险（新能源装机持续增加拉低高峰电价）；2.市场价格波动风险（现货价格不确定性）；3.政策变化风险；4.业主违约风险；5.电池衰减风险（10年运营期后容量显著下降）。'),
...
for i, (k, v) in enumerate(conclusions, 3):
    ...
    sc(ws7.cell(i,1), k, fill=LIGHT, bold=True)
    sc(ws7.cell(i,2), v, halign='left')
