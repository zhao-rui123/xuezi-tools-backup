...
def infer_status(updated_at_ms: int | None) -> str:
...
def build_card(open_id: str = DEFAULT_OPEN_ID) -> dict:
    ...
    for idx, (key, meta) in enumerate(sessions, start=1):
        ...
        blocks.append({
            ...
            'text': {
                'tag': 'lark_md',
                'content': (
                    f"**{semantic_name(key, idx)}**｜**ACP / Claude Code**｜**{infer_status(updated_at)}**\n"
    ...
    return {
        'header': {
            'title': {'tag': 'plain_text', 'content': '🧩 ACP 语义任务面板 V1'},
            'template': 'purple'
        },
        'elements': [
            {'tag': 'div', 'text': {'tag': 'lark_md', 'content': '**定位**：把 ACP 线程从 key/sessionId 翻成可读的业务任务视角。'}},
            *blocks,
            {'tag': 'action', 'actions': [
...
def main():
    parser = argparse.ArgumentParser(description='ACP 语义任务面板 V1')
