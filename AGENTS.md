...
## 🤖 CC开发任务Prompt标准模板（2026-04-03 更新）
...
## 🤖 CC任务执行最佳实践（2026-04-03 经验总结）

### 核心原则：任务拆分 + 主动检查

**问题**：大任务（如30分钟）容易超时被kill，导致进度丢失

**解决方案**：
1. **任务拆分**：每个任务控制在10分钟以内
   - 大任务拆成 2-3 个小任务
   - 例：P1-5笔记核心 → P1-5a(Note模型) + P1-5b(编辑器组件)

2. **主动检查**：每5分钟检查subagent状态
   ```bash
...
3. **即时提交**：每个小任务完成后立即git commit
   ```bash
   git add -A && git commit -m "【P1-X】模块：简短描述"
   ```

4. **主动汇报**：不等用户追问，主动报告进度
   - 完成时通知
   - 遇到问题立即汇报

**效果**：
- 之前：30分钟大任务超时被kill，进度丢失
- 之后：10分钟小任务顺利完成，进度完整
...
```
需求确认 → Opus架构设计 → MiniMax执行开发 → Opus验收审查 → 部署上线
```
...
| 文件 | 用途 |
|------|------|
| `knowledge-base/CC智能调度规则.md` | **复杂任务用详细版**（20个Agent约束、完整模板） |
| `AGENTS.md` | **普通任务用此精简版**（核心约束、决策树） |

**使用原则：**
- 普通任务 → 直接用AGENTS.md里的约束清单
- 复杂任务 → 读取详细版，组装对应Agent的完整约束

*此文件由雪子和雪子助手共同维护 - 最后更新：2026-03-31 晚间*
