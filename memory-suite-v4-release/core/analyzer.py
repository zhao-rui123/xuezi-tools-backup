...
class AnalysisManager:
    ...
    def _generate_report(self, report_type: str) -> Optional[str]:
        ...
        try:
            ...
            report = {
                ...
                "summary": self._generate_summary(report_type, date_str, len(period_files), len(memory_files))
            }
    ...
    def _generate_summary(self, report_type: str, date_str: str, period_files: int, total_files: int) -> str:
        """生成摘要文本"""
        summaries = {
            "daily": f"日报 - {date_str}: 今日产生 {period_files} 个记忆文件，累计共 {total_files} 个文件",
            "weekly": f"周报 - {date_str}: 本周产生 {period_files} 个记忆文件，累计共 {total_files} 个文件",
            "monthly": f"月报 - {date_str}: 本月产生 {period_files} 个记忆文件，累计共 {total_files} 个文件"
        }
